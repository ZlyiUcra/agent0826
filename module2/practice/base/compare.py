...
Останній запит у наборі не має відповіді в наших документах взагалі. Він показує те, чого
...
BM25 неможливо. Векторний пошук на тому ж запиті мовчить, бо його найкращий фрагмент
не дотягує до межі.
...
# Порожній рядок означає, що правильної відповіді в наших документах немає.
CASES = [
...
def main(argv: list[str]) -> int:
    ...
    for query, expected in CASES:
        print(f"Запит: «{query}»")
        print(f"  очікуємо:    {expected or 'нічого — у наших документах відповіді немає'}")
    ...
    print("Запит без відповіді в наших документах: BM25 усе одно щось віддає, "
          "ембединги мовчать, якщо найкращий фрагмент не дотяг до межі.")
    return 0
