"""
СПІЛЬНЕ · корпус документів практики і його поділ на фрагменти.
...
ЧОМУ КОРПУС — ЦЕ 18 ДОКУМЕНТІВ, А ІНДЕКСУЄМО МИ ФРАГМЕНТИ
...
запит про `String.prototype.replace` ретривер повернув би весь розділ 22.1:
...
із номером свого розділу. Корпус при цьому лишається корпусом із вісімнадцяти
документів — ділиться не він, а те, що ми з нього дістаємо.

ДЕ ПРОХОДИТЬ МЕЖА ФРАГМЕНТА

Специфікація сама пронумерована: «22.1.3.11 String.prototype.replace ( ... )»
...
# Стеля розміру фрагмента. Її задає не смак, а вікно моделі ембедингів:
...
# три символи — при стелі 1800 п'ятнадцять фрагментів із 284 вилазили за вікно
...
def _split_long(doc: Document, section: str, heading: str, text: str,
    ...
    for para in text.split("\n\n"):
        # Абзац, який сам довший за стелю (довгий алгоритм списком), лишаємо цілим:
        # різати його посередині кроку гірше, ніж перевищити стелю.
...
def split_document(doc: Document, max_chars: int = None) -> list[Passage]:
    ...
    `max_chars` перекриває стелю розміру. Потрібен лише досліду
...
def load_passages() -> list[Passage]:
    """Увесь корпус, поділений на фрагменти. Це вхід і лексичного, і векторного пошуку.
    ...
    Без злиття пошук чесно повертає обидві копії, і агент отримує два однакові
    ...
    сталий. Корпус від цього не меншає: документів як було вісімнадцять, так і
...
def fingerprint(passages: list[Passage]) -> str:
    """Відбиток корпусу — щоб кеш векторів не пережив зміну документів.
