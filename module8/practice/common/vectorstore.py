...
# Ім'я колекції несе назву набору документів і ключ моделі векторів. Набір —
# бо в колекції лежать точки саме його фрагментів; ключ моделі — бо вектори
# різних моделей непорівнянні, і змішати їх в одній колекції означало б
# отримати пошук, який мовчки шукає не те. Контейнер Qdrant тут спільний із
# практиками сусідніх модулів, тому в імені стоїть ще й номер цього.
COLLECTION = os.getenv("QDRANT_COLLECTION", f"spec8-{DOC_SET}-{MODEL_KEY}")
...
def fetch_vectors(ids: list, name: str = COLLECTION) -> dict:
    """Вектори названих точок, за їхніми номерами. Потрібно, щоб звірити збережений
    вектор із перерахованим наново — перевірка на підміну самого вектора, якої
    сума тексту не ловить."""
    res = _request("POST", f"/collections/{name}/points",
                   {"ids": ids, "with_vector": True})["result"]
    return {r["id"]: r["vector"] for r in res}
