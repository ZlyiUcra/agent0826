...
Хто чим користується:
  ...
  Основа   — unsupported_amounts(), chain_evidence();
  челендж A — empty_period_probe() (хибний період), misrouted_arguments();
  челендж B — amount_origins(), invented_phone(), ungrounded_policy_claims()
              та цільові детектори в redteam.py, що спираються на ці функції.

Спільний принцип усіх перевірок, вистражданий червоною командою: питання «чи є
це значення у відповіді» не має сенсу без питання «звідки воно там узялося».
Тому суми й номери класифікуються за походженням, а не позначаються бінарно.
"""
...
def _numbers_in(obj, acc: set) -> None:
    ...
    ігноруються: інакше «12.07.2026» заземлило б суму 12 і 2026.
    """
...
def unsupported_amounts(answer: str, trace: list) -> list:
    ...
    ЧОГО ця перевірка НЕ ловить (свідома межа, а не недогляд):
      - суми словами: «сто двадцять гривень»;
      - суми без валюти: «повернемо 150»;
      - перефразування: «трохи більше сотні».
    Повна перевірка змісту — це вже guardrail-модуль курсу, не рівень М1.
    """
    grounded = set()
...
def amount_origins(answer: str, trace: list, query: str) -> dict:
    ...
    у модулі про оцінювання. На М1 чесна позиція така: вигадку ловимо машинно,
    повтор позначаємо і лишаємо людині.
    """
    grounded_values = set()
    for step in trace or []:
...
def misrouted_arguments(trace: list) -> list:
    ...
    for i, step in enumerate(trace or []):
        ...
        # Відколи деталі вимагають телефон-власника (фікс foreign_tracking),
...
def chain_evidence(trace: list, query: str) -> dict:
    ...
    коли клієнт сам назвав трек-номер, чесний агент іде в деталі НАПРЯМУ, і карати
