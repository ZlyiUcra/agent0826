...
заливати їх удруге не треба. `spec-suite-e5` — треба, і не дозалити, а залити наново: набір спершу виріс із
3964 фрагментів до 4171, коли додали шосту й сьому частини UTS #35, а потім став 4162, коли з розділу 20
ECMA-402 прибрали помилково дописані туди додатки. Через це кожна точка після розділу 20 описує не той
фрагмент, що раніше, а практика такого не помічає — для неї колекція готова, коли точок не менше, ніж
фрагментів. Стару колекцію видаляєте ви самі, у практиці такої команди немає навмисно:
...
curl -X DELETE http://localhost:6333/collections/spec-suite-e5   # прибрати зсунуту колекцію
.venv/bin/python -m practice.challenges.qdrant_store             # залити suite наново, 4162 точки, $0
.venv/bin/python -m practice.challenges.qdrant_store --check     # звірити видачу бази з матрицею, $0
```

Грошей це не коштує, часу коштує: відбиток набору змінився, кеш векторів у `practice/index/` під нього не
підходить, і модель `e5` рахує на процесорі вектори всіх 4162 фрагментів наново — це хвилини.

Набір документів у `.env` — `suite`: уся ECMA-262 разом з ECMA-402, 404, 414 і документами довкола 402 (72
документи, 4162 фрагменти). У модулі 4 типовим стоїть `full`; тут узято все, що є, бо агентові модуля 3
