...
саме, що показує курсовий `rag_agentic.py`, тільки на своєму корпусі.
...
Підлога схожості у `VectorIndex.retrieve` відсікає запити, у яких з корпусом
немає спільної теми. «What is the recipe for borscht?» дає 0.768 при підлозі
...
підлога не спиняє: воно з тієї самої теми, що й корпус, і схожість у нього така
...
Промпт тут не перестраховка. Модель знає специфікацію ECMAScript і без нашого
корпусу: про `Array.prototype.flat` вона відповість із голови, і відповідь навіть
...
def get_index(backend: str = None):
    ...
    («vector» за замовчуванням, «lexical» — щоб прогнати агента на базовій лінії).
    """
...
def _search_once(index, query: str, k: int = 3):
    """Фрагменти, що пройшли підлогу, і оцінка найкращого з усіх — навіть якщо не пройшов."""
    top = index.scores(query, 1)
    return index.retrieve(query, k), (top[0][0] if top else 0.0)


def search_docs(query: str) -> dict:
    """Пошук по корпусу. При бідному результаті переписує запит і шукає вдруге.
    ...
    # і слова «JavaScript» у ній вистачає, щоб набрати вище підлоги. Вимір і
