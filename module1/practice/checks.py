"""
Перевірки результату, що не потребують ані мережі, ані другої моделі.

Дві штуки, обидві детерміновані:

  unsupported_amounts() — чи не назвав агент грошову суму, якої не давав жоден інструмент;
  chain_evidence()      — чи справді другий інструмент живився виходом першого.

Обидві читають `trace`, який `run_agent` уже й так повертає, тож коштують нуль токенів
і нуль секунд. Їх можна ганяти в тестах без ключа і без мережі скільки завгодно.
"""
