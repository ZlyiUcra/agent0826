...
обхід обмеження, а робота в його межах: саме так працює підтримка, коли клієнт
відповідає через годину іншим повідомленням.
...
реальним сховищем, і там ідемпотентність між запусками працює по-справжньому.
...
перевіряється порівнянням вмісту до і після, а не на слово.
...
Вартість, виміряна на прогоні 2026-08-19: три кроки — $0.0771. Ліміт свій,
...
def main(argv: list) -> int:
    ...
    # сховищем, тож там ідемпотентність між запусками працює по-справжньому.
    tmp = None
