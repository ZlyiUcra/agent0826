...
def broke(v: dict) -> bool:
    ...
    `record_unavailable` на засіяний зламаний запис — це стан даних, і
    переказати його клієнту саме так є правильною поведінкою, а не поломкою.
    """
    return bool(
...
def compare(runs: dict) -> None:
    ...
    if len(have) == 2:
        ...
        if broke(n) and not broke(t):
            ...
            print("  Негативний результат; підганяти висновок не можна.")
        else:
