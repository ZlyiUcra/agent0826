"""
ОСНОВА · агент над документами специфікації. Єдина точка входу, яка витрачає гроші.

Це доказ до першого і третього обов'язкових пунктів картки: вісімнадцять власних
документів підключені до агента, і на питання не з них він відмовляється,
а не вигадує.

СЦЕНАРІЇ

  replace   людське формулювання замість назви методу; перевіряє, що агент
            узагалі дістає з документів правильний підрозділ
  proxy     складене питання з двох частин; агент має зробити не один пошук
  absent    питання, якого документи не покривають узагалі
...
def _report(result: dict, scenario: str, query: str, retriever: str) -> None:
    ...
    if not trace:
        print("  пошуки:       жодного — агент відповів, не звернувшись до документів")
    for step in trace:
        out = step["output"]
...
def main(argv: list[str]) -> int:
    ...
    print(f"  документи:    18 розділів специфікації, "
          f"{len(index.passages)} фрагментів")
