...
Модель — `intfloat/multilingual-e5-small`, 118 мільйонів параметрів, працює
...
`BAAI/bge-small-en-v1.5`, того самого розміру, але англійська, і на цих документах
вона краща двічі. За місцем правильного розділу на п'яти запитах (вимір зроблено
в практиці модуля 2, скрипт practice/base/compare.py там) вона дає 3, 2, 20, 15, 9
проти 2, 109, 65, 107, 6 в e5 — тобто три запити з п'яти, безнадійні в e5, у неї
робочі. За відмовою вона теж краща:
...
вона розкладена на сотню мов, а наші документи англійські від першого до останнього
...
подаються з різними префіксами. Питання йде як «query: ...», фрагмент документа —
як «passage: ...». Без префіксів модель однаково щось порахує, але косинуси
зміняться, і поріг, підібраний на одному, не працюватиме на другому. Тому префікс
...
наших документів жодного стосунку. Тому нижче `THRESHOLD` пошук нічого не віддає:
порожній результат означає «у наших документах такого немає».

Значення виміряне окремо для кожної моделі скриптом practice/base/threshold.py
з практики модуля 2; сюди перенесено готові числа.
Далі йдуть числа e5, бо вона стоїть за замовчуванням; вимір показав межу самого
способу. Косинуси восьми питань, відповідь на які в документах є, лягли в
проміжок 0.831–0.883. Косинуси шести питань, яких документи не покривають, — у
...
це питання про JavaScript, а всі наші документи про JavaScript. Схожість міряє
...
Перевірено ще два способи, обидва не розділяють. Відрив найкращого фрагмента від решти:
...
Тому число в коді — це нижня межа, а не роздільна риса. Вона відсікає те, що з нашими документами не має
...
модель — вона бачить сам текст фрагментів, а не оцінку схожості. Так це було
влаштовано в практиці модуля 2, у practice/common/tools.py: нижня межа закриває
чужу тему, промпт закриває свою тему без відповіді. Тут такого файлу ще немає —
як роздати інструменти спеціалістам, вирішує сама практика модуля 4.

КЕШ

Порахувати 283 фрагменти на процесорі — це десятки секунд. Робити це на кожен
запуск немає сенсу, тому вектори лягають у practice/index/ поруч із відбитком
набору документів. Змінилися документи або межі фрагментів — відбиток інший, файл із таким
...
# Подавати текст без правильного префікса не помилка, але косинуси зміняться, і
...
# діапазонах, тож одне число на всіх не годиться. Виміряно в практиці модуля 2.
...
def _load_model():
    ...
    if _model is None:
        ...
        try:
            ...
            raise SystemExit(
                "Немає sentence-transformers. З теки module4/:\n"
...
class VectorIndex:
    """Ембединги всіх фрагментів з кешем на диску."""
    ...
    def retrieve(self, query: str, k: int = 3) -> list[Passage]:
        """Перші k з відсіканням за нижньою межею. Порожньо означає «у документах немає»."""
