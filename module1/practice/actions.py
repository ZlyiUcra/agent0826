...
def request_redirect(phone: str, tracking: str, new_city: str) -> dict:
    ...
    собі — інструмент віддавав дію кожному, хто назве номер. Прив'язка до
    телефону-власника прибирає перебір: сам трек-номер уже не ключ до дії.

    Це не повноцінна автентифікація — той, хто має пару «телефон+трек»,
    усе ще пройде. Справжня перевірка особи — рівень модуля 4; тут лише
    закрито найгрубіший вектор. Повертає заявку в стані pending або, якщо
    така вже є, її ж із позначкою already_exists.
    """
