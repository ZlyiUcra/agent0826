...
Скрипт друкує косинус найкращого фрагмента окремо для двох наборів: питання, відповідь на які
в наших документах є, і питання, яких вони не покривають. Задум був простий — поставити
поріг у проміжок між найгіршим із перших і найкращим із других.

Проміжку немає, і прогін це показує щоразу. Питання не з наших документів, але про
...
Тому число в коді працює як нижня межа: воно відрізає чужу тему й нічого не обіцяє
щодо своєї. Хто відмовляє на питання про JavaScript без відповіді в документах —
описано в practice/common/tools.py. Цей скрипт лишається як вимір: якщо документів
побільшає, перевірити, де тепер лягла межа, можна ним.
...
def main() -> int:
    ...
    inside = run("Питання, відповідь на які в документах Є:", IN_CORPUS)
    outside = run("Питання, яких документи НЕ покривають:", OUT_OF_CORPUS)
    ...
    print(f"\nнижня межа {THRESHOLD} відсікає {len(cut)} із {len(OUT_OF_CORPUS)} питань не з наших документів:")
    for s, q in cut:
        print(f"  {s:6.3f}  {q}")
    kept = [(s, q) for s, q in zip(outside, OUT_OF_CORPUS) if s >= THRESHOLD]
    print(f"долають межу {len(kept)} — на них відмовляє модель, не пошук:")
    for s, q in kept:
        print(f"  {s:6.3f}  {q}")
    return 0
