...
Цей файл — не лише точка входу основи. Усі челенджі імпортують звідси спільну
оснастку: register_tools(), enrich(), report(), save_result(), PRACTICE_PROMPT і
словник QUERIES. Тому правка тутешньої функції відгукнеться і в челенджах A, B та
D — саме через це основа лежить окремою текою base/, а не всередині common/.
"""
...
PRACTICE_PROMPT = (
    ...
    "- Якщо клієнт назвав трек-номер і свій телефон — одразу дивись деталі по ньому, "
    "пошук за телефоном не потрібен. Якщо назвав лише телефон — спершу знайди його "
    "відправлення, і лише потім дізнавайся деталі за знайденими номерами. Якщо назвав "
    "лише трек-номер — спершу спитай телефон, на який оформлено відправлення: без "
    "нього система деталей не віддає.\n"
...
def main(argv: list) -> int:
    ...
    if not state["ok"]:
        # Зупинка ДО першого виклику: коштує $0.0000 і нуль секунд.
